...
int Mesh::intersect(std::vector<Intersection> & results, const Ray & ray, bool recursive, int limit) {
    ...
    Ray localRay(ray.depth, newOrigin, newDirection, ray.strength, ray.limits.x, ray.limits.y);
            ...
            if (node->isLeaf) {
                ...
                for (int i = 0, l = indices.size(); i < l; i++) {
                    ...
                    if (IntersectTriangle(result, localRay, positions[index.x], positions[index.y],
                            positions[index.z])) {
                        results.push_back(Intersection(result.w / tScale, this, index, glm::vec3(result)));
                        count++;
                ...
                // Add to stack
                std::vector<Accel *> & nodeChildren = node->children;
                for (int i = 0, l = nodeChildren.size(); i < l; i++) {
                    accelStack.push(nodeChildren[i]);
                }
            }
        }
    }
